        ...
        try {
            ...
            if (resultSet.next()) {
                ...
                while (!links.isEmpty()) {
                    ...
                    //и убираем из него, добавив через доп. переменную в ArrayList books
                    int link0 = links.get(0);
                    links.remove(0);
                    if (!books.contains(link0)) {
                        books.add(link0);

                        //после чего запускаем рекурсию
                        readBook(link0, false);
                    }
                }
                //если никаких ссылок нет ниже по дереву и нужно вывести сообщение - выводим его
                if (books.isEmpty() && message)
                    System.out.println("Вы можете прочитать эту книгу");

            //если такой книги нет
            } else { System.out.println("Такой книги нет в библиотеке: " + bookID); }
